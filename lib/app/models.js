...
export const models = {
  state: {
    ...
    lightenStepValue: 10,
    darkenStepValue: 10,
    saturateStepValue: 10,
    desaturateStepValue: 10
  },
    ...
    [SET_COLOR]({color}, state) {
      ...
      const nextState = Object.assign({}, state, {
        ...
        lightenColors: Array(6).fill().map((item, idx) => (
          color.clone().lighten((idx + 1) * state.lightenStepValue)
        )),
        darkenColors: Array(6).fill().map((item, idx) => (
          color.clone().darken((idx + 1) * state.darkenStepValue)
        )),
        saturateColors: Array(6).fill().map((item, idx) => (
          color.clone().saturate((idx + 1) * state.saturateStepValue)
        )),
        desaturateColors: Array(6).fill().map((item, idx) => (
          color.clone().desaturate((idx + 1) * state.desaturateStepValue)
        ))
      });

      nextState.history.unshift(color.clone());
      return nextState;
    },
  ...
  effects: {
    [SET_STEP_VALUE]({headline, value}, state, send, done) {
      if (headline === 'Lighten') {
        state.lightenStepValue = Number(value);
      } else if (headline === 'Darken') {
        state.darkenStepValue = Number(value);
      } else if (headline === 'Saturate') {
        state.saturateStepValue = Number(value);
      } else if (headline === 'Desaturate') {
        state.desaturateStepValue = Number(value);
      }

      send(SET_COLOR, {color: state.original}, done);
    },
    ...
    (send, done) => {
      subscriptions.add(atom.commands.onDidDispatch(ev => {
        if (ev.type !== 'core:copy' ||
            !atom.config.get('colorio.searchOnCopy')) {
          return;
        }
